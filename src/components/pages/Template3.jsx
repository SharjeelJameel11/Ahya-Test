...
const Sidebar = ({ resumeData }) => {
  ...
  return (
    <div className="col-span-2 border-b-[10px] border-[#5DD3B6] relative bg-[#ffffff] py-10 px-6 flex flex-col items-center">
      <div className="absolute top-0 left-0 w-full border-t-[10px] border-[#5DD3B6]" />
        ...
        <div className="w-40 h-40 bg-white p-1 rounded-full">
          {personal?.photo ? (
            <img
              src={personal.photo}
              alt="Profile"
              className="w-full h-full object-cover border-[3px] border-[#5DD3B6] rounded-full"
            />
          ) : (
            <div className="w-full h-full bg-gray-200 border-[3px] border-[#5DD3B6] rounded-full" />
          )}
        </div>
      </div>

      <div className="relative z-10 w-full space-y-10 text-sm text-gray-700">
...
const MainContent = ({ resumeData }) => {
  ...
  return (
    <div className="col-span-3 relative px-10 py-8 bg-[#F9F9F9]">
      <div className="absolute top-0 right-0 h-[30%] w-[10px] bg-[#5DD3B6]" />
      <div className="mb-10">
...
const Section = ({ title, Icon, children }) => (
  <div>
    <div className="flex items-center gap-3 mb-3">
      <div className="w-8 h-8 bg-[#5DD3B6] rounded-full flex items-center justify-center text-white">
        <Icon />
      </div>
...
const MainSection = ({ title, children }) => (
  <div className="mb-10">
    <div className="flex items-center gap-3 mb-4">
      <span className="w-3 h-3 rounded-full bg-[#5DD3B6]" />
